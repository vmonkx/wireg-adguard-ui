...
printf "${BLUE} Сейчас проверим свободен ли порт 62087 и не установлен ли другой wireguard.\n${NC}"

if [[ $(docker ps -q --filter "name=wireguard") ]]; then
    printf "!!!!>>> Другой Wireguard контейнер уже запущен, и вероятно занимает порт 62087. Пожалуйста удалите его и запустите скрипт заново\n "
    printf "${RED} !!!!>>> Завершаю скрипт! \n${NC}"
    exit 1
else
    printf "Wireguard контейнер не запущен в докер. Можно продолжать\n"
    # Проверка, запущен ли контейнер, использующий порт 62087
    if lsof -Pi :62087 -sTCP:LISTEN -t >/dev/null ; then
        printf "${RED}!!!!>>> Порт 62087 уже используется контейнером.!\n ${NC}"
        if docker ps --format '{{.Names}} {{.Ports}}' | grep -q "wg-easy.*:62087->" ; then
            printf "WG-EASY контейнер использует порт 62087. Хотите продолжить установку? (y/n): "
            read -r choice
            case "$choice" in 
...
echo -e "Адрес входа в веб-интерфейс WireGuard после установки: ${YELLOW}http://$CURRENT_WG_HOST:62087${NC}"
echo ""
...
echo ""
echo -e "Адрес входа в веб-интерфейс WireGuard после установки: ${BLUE}http://$CURRENT_WG_HOST:62087${NC}"
echo -e "Адрес входа после настройки UFW-Docker, для подключения к Wireguard: ${BLUE}http://wg.local:62087${NC}"
echo -e "Пароль от веб-интерфейса: ${BLUE}$CURRENT_PASSWORD${NC}"
echo ""

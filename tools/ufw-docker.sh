...
# Разрешаем трафик на порт 62087 для сети 10.10.10.0/24
sudo ufw route allow proto tcp from 10.10.10.0/24 to any port 62087
  printf "${GREEN}Разрешаем трафик на порт 62087 только для внутренней сети докера${NC}\n"
# Отключаем ufw
sudo ufw disable
...
sudo ufw --force enable
  ...
  printf "${YELLOW}***********************${NC}\n"
    printf "${RED}Теперь веб-интерфейс будет доступен только по адресу (ТОЛЬКО ПРИ ПОДКЛЮЧЕНИИ ЧЕРЕЗ WIREGUARD!):${NC}\n"
      printf "${BLUE}http://10.2.0.3:62087${NC}\n"
        printf "${YELLOW}***********************${NC}\n"
